     ...
     <div class="card-body">
      <div class="row">
       ...
       <div class="col-md-4 d-flex flex-row align-items-center justify-content-end">
        <div class="form-group m-0"><a class="btn btn-primary" style="width:240px;" id="btnFiltrar">Buscar</a>
        </div>
       </div>
      </div>
     </div>
...
<script>
	...
	$(document).ready(function() {
		...
		table = $('#lstVentas').DataTable({
			...
			ajax: {
				...
				"dataSrc": function(respuesta) {
					...
					$("#totalVenta").html(TotalVenta.toFixed(1).toString().replace(/\d(?=(\d{3})+\.)/g, "$&,"));
					return respuesta;
				}
			},
			drawCallback: function(settings) {
				...
				api.column(groupColumn, {
					...
					if (last !== group) {
						...
						$(rows).eq(i).before(
							'<tr class="group">' +
							'<td colspan="6" class="fs-6 fw-bold bg-success text-white"> ' +
		...
		/* ================================================================
						EVENTO PARA FILTRAR VENTAS SEGUN EL RANGO SELECCIONADO
		=================================================================*/
		$("#btnFiltrar").on('click', function() {
			table.destroy();
			if ($("#ventas_desde").val() == '') {
				ventas_desde = '01/01/2024';
			} else {
				ventas_desde = $("#ventas_desde").val();
			}
			...
			table = $('#lstVentas').DataTable({
				...
				ajax: {
					...
					"dataSrc": function(respuesta) {
						...
						$("#totalVenta").html(TotalVenta.toFixed(0));
						return respuesta;
					},
					data: {
						'accion': 2,
