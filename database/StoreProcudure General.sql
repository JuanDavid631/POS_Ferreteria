...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-05-2024 a las 05:48:06
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ListarProductos` ()   BEGIN
	SELECT '' AS detalles, p.id, p.codigo_producto, c.id_categoria, c.nombre_categoria, p.descripcion_producto, 
		CONCAT('$ ', FORMAT(ROUND(p.precio_compra_producto, 1), 0)) AS precio_compra,
		CONCAT('$ ', FORMAT(ROUND(p.precio_venta_producto, 1), 0)) AS precio_venta,
		CONCAT('$ ', FORMAT(ROUND(p.utilidad, 1), 0)) AS utilidad,
        ...
        -- CONCAT(SUM(vd.cantidad), ' Und(s)') AS cantidad, '$ ', FORMAT(SUM(ROUND(vd.total_venta, 1)), 0) AS total_venta 
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ListarProductosMasVendidos` ()   BEGIN
	SELECT p.codigo_producto, p.descripcion_producto, CONCAT(SUM(vd.cantidad), ' Und(s)') AS cantidad, '$ ', FORMAT(SUM(ROUND(vd.total_venta, 1)), 0) AS total_venta 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ListarProductosVentas` ()   BEGIN
	SELECT CONCAT(codigo_producto , ' - ' , c.nombre_categoria, ' - ', descripcion_producto, ' - $ ' , FORMAT(ROUND(p.precio_venta_producto, 1), 0))  
		AS descripcion_producto FROM productos p INNER JOIN categorias c ON p.id_categoria_producto = c.id_categoria;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ListarUsuarios` ()   BEGIN
	SELECT '' AS detalles, '' AS opciones, usu.id_usuario, usu.nombre_usuario, usu.apellido_usuario, usu.usuario, usu.id_perfil_usuario, p.descripcion AS perfil, CASE WHEN usu.estado = 1 THEN 'Activo' ELSE 'Inactivo' END AS estado 
    	FROM usuarios usu INNER JOIN perfiles p ON usu.id_perfil_usuario = p.id_perfil;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_moduloSistema` ()   BEGIN
SELECT '' AS opciones, id, orden, modulo,
	(SELECT modulo FROM modulos mp WHERE mp.id = m.padre_id) AS modulo_padre, vista, icon_menu,
    DATE(fecha_creacion) AS fecha_creacion, DATE(fecha_actualizacion) AS fecha_actualizacion
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_modulosPerfilesData` ()   BEGIN
	SELECT p.id_perfil, p.descripcion, p.estado FROM perfiles p ORDER BY p.id_perfil;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ObtenerDatosDashboard` ()   BEGIN
	...
	SELECT
    	...
    	IFNULL(CONCAT('$ ', FORMAT(totalCompras, 1)), 0) AS totalCompras,
		IFNULL(CONCAT('$ ', FORMAT(totalVentas, 1)), 0) AS totalVentas,
    	IFNULL(CONCAT('$ ', FORMAT(ganancias, 1)), 0) AS ganancias,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_RegistrarModulo` ()   BEGIN
	SELECT max(orden) FROM modulos m;
END$$
...
(1, 'Corona', 1014300829, 'Calle 80 #17 - 32, Barrio Polo', 'Corona Pisos', '0002', '00000039', 'ventas_pisos@corona.com');
...
(1, 'Tablero Principal', 0, 'dashboard.php', 'fas fa-tachometer-alt', 0, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(2, 'Ventas', 0, NULL, 'fas fa-store-alt', 5, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(3, 'Punto de Venta', 2, 'ventas.php', 'far fa-circle', 6, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(4, 'Administrar Ventas', 2, 'administrar_ventas.php', 'far fa-circle', 7, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(5, 'Productos', 0, NULL, 'fas fa-cart-plus', 1, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(6, 'Inventario', 5, 'productos.php', 'far fa-circle', 2, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(7, 'Carga Masiva', 5, 'cargaMasiva_Productos.php', 'far fa-circle', 3, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(8, 'Categorías', 5, 'categorias.php', 'far fa-circle', 4, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(11, 'Configuración', 0, NULL, 'fas fa-cogs', 8, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(12, 'Usuarios', 11, 'usuarios.php', 'fas fa-users', 9, '2024-05-02 03:28:31', '2024-05-02 22:28:39'),
(13, 'Modulos y Perfiles', 11, 'modulos_perfiles.php', 'fas fa-tablet-alt', 10, '2024-05-02 03:28:31', '2024-05-02 22:28:39');
...
(7, 'Seguridad', 1, '2024-05-04 05:00:00', '2024-05-04 00:00:00'),
(11, 'Salud', 1, '2024-05-04 05:00:00', '2024-05-04 00:00:00');
...
(596, 101010, 405, 'Luces Techo Led', 12000, 24000, 12000, 15, 2, 38, '2024-05-04 16:06:33', '2024-04-28', 22000, 21500),
(597, 20202, 1, 'Varsol', 2000, 5000, 3000, 36, 8, 58, '2024-05-04 16:06:33', '2024-04-28', 3000, 2500),
(599, 303030, 456, 'Llaves de Seguridad', 20000, 45000, 25000, 7, 5, 16, '2024-04-30 01:17:16', '2024-04-30', 43000, 42500),
(623, 40404040, 463, 'Liquido Tiner', 3000, 5700, 2700, 9, 3, 6, '2024-05-04 07:06:47', '2024-04-30', 3700, 3200),
(624, 5050505, 450, 'Pantallas', 3000, 7900, 4900, 11, 1, 0, '2024-05-04 07:24:28', '2024-05-04', NULL, NULL);
...
(3, 'Camila', 'Fernandez', 'cfernandez', '$2a$07$azybxcags23425sdg23sdeanQZqjaf6Birm2NvcYTNtJw24CsO5uq', 3, 1);
...
(70, '00000038', 'Venta realizada con Nro Boleta: 00000038', 0, 0, 81600, '2024-05-04 06:28:45'),
(71, '00000039', 'Venta realizada con Nro Boleta: 00000039', 0, 0, 145000, '2024-05-04 16:06:33');
...
(663, '00000038', 101010, 3, 72000, '2024-05-04 06:28:45'),
(664, '00000038', 40404040, 3, 9600, '2024-05-04 06:28:45'),
(665, '00000039', 101010, 5, 120000, '2024-05-04 16:06:33'),
(666, '00000039', 20202, 5, 25000, '2024-05-04 16:06:33');
...
--
ALTER TABLE `perfil_modulo`
  MODIFY `idperfil_modulo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=189;
...
--
ALTER TABLE `venta_cabecera`
  MODIFY `id_boleta` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=72;
